...
* typ2 (bezkontextova):
    *  `A-> &alpha;  A &isin; N , &alpha; &isin; (N sjednoceno &Sigma;)^+`

* typ3 (kontextove):
    * `&alpha; A &beta; -> &alpha; &gama; &beta;, kde &alpha;, &beta; &isin; do (N sjednoceno &Sigma;)^+`,
       &gama; &isin; do `(N sjednoceno &Sigma;)^+ prip s->&epsilon;`
       ...
...
```
&forall; q1,q2 &isin; Q &forall; w1, w2 &isin; &Sigma;* : (q1, q1)|M- (q2, w2) <=def=>
 &exist; a &isin; &Sigma;: q2 &isin; Delta(q1, a) and w1=aw2
(muzu prejit z q1 do q2 za prijeti w1 pres a a dostanu w2)
```

* definice primaneho jazyka
  `L(M)={w &isin; &Sigma;* | (q0,w) |*- (q_f, &epsilon;), q_f &isin; F}`
...
### priklad
sestavte konecny automat ktery prima jazyk `{awb &isin; {a,b}^* | |awb|=3k , k>=0}`
...
```
 =k= je podmozinou QxQ . &forall; q1, q2 &isin; Q . q1 =k= q2 <=def=> &not; (&not; &exist; w &isin; &Sigma;* &exist; q3 q4 &isin; Q 
  |w| <= k 
  and (q1, w) |k- (q3, &epsilon;) and (q2,w) |k)(q4, &epsilon;)
  and ((q3 &isin; F and q4 &notin; F) nebo (q3 &notin; FF and q4 &isin;))
```
